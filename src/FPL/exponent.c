...
FPL_float64 static polynomial(double x){
...
FPL_float64 FPL_exponent_64(FPL_float64 expr){
	...
	int n = FPL_to_integer(FPL_division_64(expr,LOGN2)); //TODO to_integer to powinien być round()
	FPL_float64 y = FPL_subtraction_64(expr, FPL_multiplication_64(FPL_double_to_float64((double)n), LOGN2));
	int i = FPL_to_integer(FPL_multiplication_64(y,FPL_double_to_float64((double)512))); //not sure czy to ucina, powinien być odpowiednik round(całość) w miejscu to_integer()
